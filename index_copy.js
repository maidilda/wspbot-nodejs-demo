...
client.on('message', msg => {
    ...
    */
    ...
    /*
    ** El primer mensaje - La condicional es debe ser "hola"
    */
    if (isFirstTime && msg.body != '') {
        client.sendMessage(msg.from, `Hola soy [nombre], soy una bot programada para orientar tanto a mujeres como hombres a traves de diversos temas relacionados con la brecha de desigualdad que presenta Paraguay en cuanto al genero
        Te puedo guiar por diversos temas relacionados:
        1. *Noticias y Estadisticas:* ¡Conocer y comprender la realidad actual es el primer paso para el cambio!
        2. *Mujeres Inspiradoras:* Buscas inspiracion? a pesar de todo el estigma social estas mujeres son simbolo de superacion
    ...
    */ 
    else if (!isFirstTime && lower_msg === '1') {
        client.sendMessage(msg.from, `Elige entre:
    - EQUIDAD: Desafios de la mujer paraguaya
    ...
    ** OPCIONES
    */
    else if (!isFirstTime && lower_msg === 'equidad') {
    ...
    _Para finalizar responda con "salir"_`)
        ...
        ** MAS OPCIONES
        */
        else if (!isFirstTime && lower_msg === "a") {
        ...
        else if (!isFirstTime && lower_msg === "b") {
        ...
        else if (!isFirstTime && lower_msg === "c") {
    ...
    } else if (!isFirstTime && lower_msg === 'brecha') {
    client.searchMessages(msg.from, `Según la UNESCO, los hombres tienen 25% más probabilidad de contar con los conocimientos y habilidades para usar la tecnología que las mujeres.
    Asimismo, según la CEPAL en América Latina, 40% de las mujeres no están conectadas o no pueden costear el acceso al Internet.
    ...
    } else if (!isFirstTime && lower_msg === 'conexion') {
    client.searchMessages(msg.from, `Es una herramienta que hace posible que datos relacionados a violencia contra las mujeres estén sistematizados, visibilizados y contribuyan a mejorar las políticas públicas de prevención y de cuidado.
    ...
    */
    else if(!isFirstTime && lower_msg === '2') {
    ...
    **OPCIONES
    */
    else if (!isFirstTime && lower_msg === 'cambio') {
    ...
    } else if(!isFirstTime && lower_msg === 'ciencia') {
        client.sendMessage(msg.from, `*CIENCIA: Alejandra Fresco*
        Pamela BovedaEs científica investigadora, astrónoma y actualmente trabaja en el ámbito de estas disciplinas en el Instituto Max Planck de Alemania. “Básicamente, trabajo estudiando el universo”, recalca.
        
        _Para finalizar responda con "salir"_`);
    
    }  else if(!isFirstTime && lower_msg === 'arte') {
    ...
    */
    else if(!isFirstTime && lower_msg === '3') {
        ...
        Para saber mas responda con "COMUNIDAD", "MINISTERIO", "EPEP" o 0 para volver al inicio
        _Para finalizar responda con "salir"_
        `);
    ...
    **OPCIONES
    */
    else if(!isFirstTime && lower_msg === 'comunidad') {
        client.sendMessage(msg.from, `*COMUNIDAD: Ciudad Mujer*
        Ciudad Mujer busca promover el empoderamiento y la autonomía real de las mujeres  a través de servicios de salud sexual y reproductiva, apoyo para el empleo y para empezar o mejorar emprendimientos propios, además de asesoramiento, contención y asistencia ante cualquier caso de violencia contra la  mujer.
        
        _Para finalizar responda con "salir"_`)
    } else if(!isFirstTime && lower_msg === 'ministerio') {
    ...
    } else if(!isFirstTime && lower_msg === 'apep') {
    ...
    **OPCIONES
    */
    else if(!isFirstTime && lower_msg === 'autonomia') {
    ...
    else if(!isFirstTime && lower_msg === 'snpp') {
    ...
    */
    ...
    client.sendMessage(msg.from, `Te puedo guiar por diversos temas relacionados. Sobre que te gustaria conocer?
    1. *Noticias y Estadisticas:* ¡Conocer la situacion actual en cuanto a cifras y el impacto que tienen nuestras acciones es el primer paso para el cambio!
    2. *Mujeres Inspiradoras:* Buscas inspiracion? a pesar de todo el estigma social estas mujeres son simbolo de superacion
    3. *Fundaciones:* El apoyo entre mujeres es esencial para romper con las dificultades que se presentan, te muestro algunas de las fundaciones de mujeres para mujeres. 
    4. *Talleres:* ¡Unicamente aprendiendo mas sobre el mundo podemos entenderlo y cambiarlo!
   ...
   //?DESPEDIDA
   else if(!isFirstTime && lower_msg === 'salir'){
